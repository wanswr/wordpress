...
foreach ([5995, 5939, 1040, 6023, 6024, 6025, 6026] as $id) { // Теневые, Световые линии, Треки (3 типа), Безрамные, Лайтбоксы
    add_m($menu_id, get_the_title($id), $id, $h_design);
}

// 3. Потолки - виды
$h_types = add_m($menu_id, 'Потолки');
foreach ([5934, 5935, 5936, 5937, 5941, 5940, 5943, 5996, 5997] as $id) { // Матовые, Глянцевые, Сатиновые, Тканевые, Двухуровневые, Звездное небо, С подсветкой, Световые, Фотопечать
...
// 5. Калькулятор (Assuming 5932 is calculation related or use custom if page missing)
add_m($menu_id, 'Калькулятор', 5932);

// 6. Портфолио (Using home or custom if missing)
add_m($menu_id, 'Портфолио', 309); // Using Catalog as portfolio placeholder
