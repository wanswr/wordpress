...
class Components extends \WBCR\Factory_Templates_759\Pages\PageBase {
	...
	public function __construct( \Wbcr_Factory600_Plugin $plugin ) {
		$this->menu_title                  = __( 'Components', 'robin-image-optimizer' );
		$this->page_menu_short_description = __( 'More features for plugin', 'robin-image-optimizer' );
	...
	public function assets( $scripts, $styles ) {
		...
		$this->styles->add( FACTORY_TEMPLATES_459_URL . '/assets/css/components.css' );
	 ...
	 *
	 * @return array
	 * @see libs\factory\pages\themplates\FactoryPages600_ImpressiveThemplate
	 */
	public function getActionNotices( $notices ) {
		$notices[] = [
			...
			'type'       => 'success',
			'message'    => __( 'Components have been successfully updated to the latest version.', 'robin-image-optimizer' ),
		];
		...
		$notices[] = [
			...
			'type'       => 'danger',
			'message'    => __( 'To use premium components, you need activate a license!', 'robin-image-optimizer' ) . '<a href="admin.php?page=license-wbcr_clearfy" class="btn btn-gold">' . __( 'Activate license', 'robin-image-optimizer' ) . '</a>',
		];
		...
		$notices[] = [
			...
			'type'       => 'danger',
			'message'    => __( 'An unknown error occurred while updating plugin components. Please contact the plugin support team to resolve this issue.', 'robin-image-optimizer' ),
		];
	...
	public function showPageContent() {
		...
		<div class="wbcr-factory-page-group-header">
			<strong>
				<?php _e( 'Plugin Components.', 'robin-image-optimizer' ); ?>
			</strong>
			<p>
				<?php _e( 'These are components of the plugin bundle. When you activate the plugin, all the components turned on by default. If you don’t need some function, you can easily turn it off on this page.', 'robin-image-optimizer' ); ?>
			</p>
		</div>
		<div class="wbc-factory-templates-459-components">
			<?php
			/**
			 * @since 1.4.0
			 */
					...
					<div class="plugin-card-bottom">
						<?php if ( ! ( isset( $component['disable_button'] ) && $component['disable_button'] ) ) : ?>
							<?php if ( 'premium' === $component['build'] && ! ( $this->plugin->premium->is_activate() && $this->plugin->premium->is_install_package() ) ) : ?>
								<a target="_blank" href="<?php echo esc_url( $component['url'] ); ?>" class="button button-default install-now"><?php _e( 'Read more', 'robin-image-optimizer' ); ?></a>
							<?php else : ?>
