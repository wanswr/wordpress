...
if ( ! class_exists( 'Wbcr_FactoryForms600_WpEditorControl' ) ) {

	class Wbcr_FactoryForms600_WpEditorControl extends Wbcr_FactoryForms600_Control {
		...
		protected function beforeHtml() {
			...
			if ( empty( $this->options['tinymce'] ) ) {
				$this->options['tinymce'] = [];
			}

			if ( ! isset( $this->options['tinymce']['content_css'] ) ) {
				$this->options['tinymce']['content_css'] = FACTORY_FORMS_600_URL . '/assets/css/editor.css';
			}
		}
				...
				<?php
					wp_editor(
						...
						[
							...
							'wpautop'       => false,
							'teeny'         => true,
							'tinymce'       => $this->getOption( 'tinymce', [] ),
