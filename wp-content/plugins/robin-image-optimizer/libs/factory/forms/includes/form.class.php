...
if ( ! defined( 'ABSPATH' ) ) {
	...
	add_action( 'wbcr_factory_forms_600_plugin_created', 'wbcr_factory_forms_600_plugin_created' );

function wbcr_factory_forms_600_plugin_created( $plugin ) {
	$plugin->forms = new Wbcr_FactoryForms600_Manager( $plugin );
}

if ( ! class_exists( 'Wbcr_FactoryForms600_Manager' ) ) {

	class Wbcr_FactoryForms600_Manager {
		 ...
		 * @var array
		 */
		public static $registered_controls = [];
		 ...
		 *
		 * @see FactoryForms600_Form::registerControl()
		 *
		 * @since 1.0.0
		 * @return void
		 */
		 ...
		 * @var array
		 */
		public static $registered_holders = [];
		 ...
		 *
		 * @see FactoryForms600_Form::registerHolder()
		 *
		 * @since 1.0.0
		 * @return void
		 */
		 ...
		 * @var array
		 */
		public static $registered_custom_elements = [];
		 ...
		 *
		 * @see FactoryForms600_Form::registerCustomElement()
		 *
		 * @since 1.0.0
		 * @return void
		 */
...
if ( ! class_exists( 'Wbcr_FactoryForms600_Form' ) ) {
	/**
	 * An abstraction for forms.
	 */
	class Wbcr_FactoryForms600_Form {
		 ...
		 *
		 * @since 1.0.0
		 * @var Wbcr_IFactoryForms600_ValueProvider
		 */
		private $provider;
		 ...
		 *
		 * @since 1.0.0
		 * @var Wbcr_FactoryForms600_FormLayout
		 */
		public $layout;
		 ...
		 */
		public function __construct( array $options, Wbcr_Factory600_Plugin $plugin ) {
			// register controls once, when the first form is created
			if ( ! Wbcr_FactoryForms600_Manager::$controls_registered ) {

				do_action( 'wbcr_factory_forms_600_register_controls', $plugin );

				// if( !empty($plugin) ) {
				do_action( 'wbcr_factory_forms_600_register_controls_' . $plugin->getPluginName(), $plugin );
				// }

				Wbcr_FactoryForms600_Manager::$controls_registered = true;
			}
			...
			Wbcr_FactoryForms600_Manager::$temper = 'flat';
		}
		 ...
		 *
		 * @since 1.0.0
		 * @param Wbcr_IFactoryForms600_ValueProvider $provider
		 * @return void
		 */
		...
		public function getItems() {
			return apply_filters( 'wbcr_factory_600_form_items', $this->items, $this->name );
		}
		 ...
		 *
		 * @since 1.0.0
		 * @return Wbcr_FactoryForms600_Control[]
		 */
		 ...
		 * @return Wbcr_FactoryForms600_FormElement|null A form element.
		 */
		 ...
		 * @return Wbcr_FactoryForms600_FormElement[] Created elements.
		 */
		public function createElements( $items = [] ) {
			$objects = [];
			foreach ( $items as $item ) {
		 ...
		 * @since 1.0.0
		 * @param array $item Item data.
		 * @return Wbcr_FactoryForms600_Control A control object.
		 */
		public function createControl( $item ) {
			...
			if ( is_array( $item ) ) {
				...
				$control_data = Wbcr_FactoryForms600_Manager::$registered_controls[ $item['type'] ];
		 ...
		 * @since 1.0.0
		 * @param array $item Item data.
		 * @return Wbcr_FactoryForms600_Holder A control holder object.
		 */
		public function createHolder( $item ) {
			...
			if ( is_array( $item ) ) {
				...
				$holderData = Wbcr_FactoryForms600_Manager::$registered_holders[ $item['type'] ];
		 ...
		 * @return Wbcr_FactoryForms600_FormElement A custom form element object.
		 */
		public function createCustomElement( $item ) {
			...
			if ( is_array( $item ) ) {
				...
				$data = Wbcr_FactoryForms600_Manager::$registered_custom_elements[ $item['type'] ];
		...
		public function html( $options = [] ) {
			...
			if ( ! isset( Wbcr_FactoryForms600_Manager::$form_layouts[ $this->form_layout ] ) ) {
				die( sprintf( '[ERROR] The form layout %s was not found.', $this->form_layout ) );
			}

			// include a render code
			$layout_data = Wbcr_FactoryForms600_Manager::$form_layouts[ $this->form_layout ];
		 ...
		 * @return void
		 */
		public static function connectAssetsForItems( $items = [] ) {
			foreach ( $items as $item ) {
		...
		public static function connectAssetsForItem( $item ) {
			...
			if ( self::isControl( $type ) ) {
				$haystack = Wbcr_FactoryForms600_Manager::$registered_controls;
			} elseif ( self::isControlHolder( $type ) ) {
				$haystack = Wbcr_FactoryForms600_Manager::$registered_holders;
			}
			...
			if ( isset( $haystack[ $type ] ) ) {
				...
				if ( isset( $haystack[ $type ]['script'] ) ) {
					...
					if ( ! wp_script_is( $script ) ) {
						wp_enqueue_script( 'factory-form-control-' . $type, $script, [ 'jquery' ] );
					}
				}
			}
		...
		public static function isControl( $item ) {
			return isset( Wbcr_FactoryForms600_Manager::$registered_controls[ $item['type'] ] );
		}
		...
		public static function isControlHolder( $item ) {
			return isset( Wbcr_FactoryForms600_Manager::$registered_holders[ $item['type'] ] );
		}
		...
		public static function isCustomElement( $item ) {
			return isset( Wbcr_FactoryForms600_Manager::$registered_custom_elements[ $item['type'] ] );
		}
	}
}
