...
if ( ! class_exists( 'Wbcr_FactoryForms600_Bootstrap3FormLayout' ) ) {
	/**
	 * A form layout based on Twitter Bootstrap 3
	 */
	class Wbcr_FactoryForms600_Bootstrap3FormLayout extends Wbcr_FactoryForms600_FormLayout {
		 ...
		 * @param Wbcr_FactoryForms600_Form $form A parent form.
		 */
		...
		/**
		 * @param Wbcr_FactoryForms600_Control $control
		 */
		public function beforeControl( $control ) {
			...
			if ( isset( $control->options['theme'] ) ) {
				$theme_class = strval( $control->options['theme'] );
			}

			$control_name       = $control->getOption( 'name' );
			$control_name_class = $control_name
				? 'factory-control-' . strval( $control_name )
				: '';

			$col_left  = $control->getLayoutOption( 'column-left', '2' );
			$col_right = $control->getLayoutOption( 'column-right', '10' );
			?>
				<div class="form-group form-group-<?php echo esc_attr( strval( $control->getType() ) ); ?> <?php echo esc_attr( $theme_class ); ?> <?php echo esc_attr( $control_name_class ); ?>">
				<label <?php echo $control->getType() !== 'checkbox' ? 'for="' . esc_attr( strval( $control->getNameOnForm() ) ) . '"' : ''; ?> class="col-sm-<?php echo esc_attr( $col_left ); ?> control-label">
