...
class Freemius_Api_WordPress extends Freemius_Api_Base {
	...
	function GetSignedUrl( $pPath ) {
		...
		return self::GetUrl( $pResourceUrl . '?' . ( 1 < count( $resource ) && ! empty( $resource[1] ) ? $resource[1] . '&' : '' ) . 'authorization=' . urlencode( $auth['authorization'] ) . '&auth_date=' . urlencode( $auth['date'] ), $this->_isSandbox );
	}
	...
	private static function MakeStaticRequest( $pCanonizedPath, $pMethod = 'GET', $pParams = [], $pWPRemoteArgs = null, $pIsSandbox = false, $pBeforeExecutionFunction = null ) {
		...
		} elseif ( FREEMIUS_SDK__SIMULATE_NO_API_CONNECTIVITY_SQUID_ACL ) {
				...
				if ( preg_match( $regex, $result->get_error_message( 'http_request_failed' ), $matches ) ) {
					...
					if ( filter_var( $matches[1], FILTER_VALIDATE_IP ) ) {
						if ( strlen( inet_pton( $matches[1] ) ) === 16 ) {
							...
							add_action( 'http_api_curl', 'WBCR\Factory_Freemius_Rio_600\Sdk\Freemius_Api_WordPress::CurlResolveToIPv4', 10, 1 );
		...
		if ( is_null( $decoded ) ) {
			...
			} elseif ( preg_match( '/Access control configuration prevents your request from being allowed at this time. Please contact your service provider if you feel this is incorrect./', $response_body ) && preg_match( '/squid/', $response_body ) ) {
